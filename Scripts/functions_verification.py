...
def read_in_allfiles(folder):
    files = [folder+filename for filename in rm_point(os.listdir(folder))]
    merged_data=xr.open_mfdataset(files,engine="netcdf4")
    return merged_data
    


def read_in_files_by_year(startyear,endyear,folder,pr=True):
    ...
    for i in range(len(list_of_starts)):
        ...
        if locals()[name2 + year]:
            ...
            locals()[name + year]=xr.open_mfdataset(locals()[name2 + year],combine="nested",concat_dim="time",engine="netcdf4")
            data=xr.open_mfdataset(locals()[name2 + year],combine="nested",concat_dim="time",engine="netcdf4")
...
def ver_plotting_pp(dataset1,dataset2,var1,output_folder,cc="depth",grid=True):
    ...
    fig.savefig(os.path.join(output_folder,today_string+" "+title+" "+color_code+".png"))
    plt.close('all')
...
def ver_plotting_tchain(dataset1,dataset2,var1,output_folder,cc="depth",grid=True):
    today_string=datetime.today().strftime('%Y%m%d')
    z=dataset1.depth.values
    z_scaled=(z-z.min())/z.ptp()
    colors=plt.cm.viridis(z_scaled,len(z))
...
def plot_clim(dataset,var,output_folder,cmap="inferno",res="monthly"):
    ...
    plt.title(title)
    fig.savefig(os.path.join(output_folder,today_string+" "+title+".pdf"))
    fig.savefig(os.path.join(output_folder,today_string+" "+title+".png"))
    plt.close('all')
